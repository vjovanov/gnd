...
- `grund show <ID> --brief` — for a bare cited `{MARKER}<ID>`, read the lead prose plus section map first.
...
- **Refresh cited specs before editing code.** Before editing code that carries a `{MARKER}<ID>` or `{MARKER}<ID>.<section>` citation, run `grund show <ID> --brief` for a bare ID or `grund show <ID>.<section>` for a section citation; escalate to `grund show <ID>` only when the slice is not enough.
