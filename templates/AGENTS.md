...
1. Start with the project knowledge map below, then open the relevant declaration with `grund show <ID>` (or `grund show <ID>.<section>` when the citation names a section) before making changes; use `grund show <ID> --toc` when you need the section map.
...
The `grund` ID scheme in this repo: `{ID_SHAPE_SEC}`, where `KIND` ∈ `{KINDS_SET}` — the kinds and the ID/marker syntax are configurable in `.agents/grund.toml` (run `grund config show` to see the effective settings). Citations are written prefixed by the marker `{MARKER}` — type `{TRIGGER}` in a `grund`-aware editor and it becomes `{MARKER}` automatically. {BARE_TOKEN_NOTE}
...
Code back-references the spec it realizes. Cite the most-specific point: a whole behavior on the function/class/block doc-comment, a clause or decision (`{MARKER}<ID>.2.1`) inline where it is enforced. Each citation is one more edge `grund refs <ID>` reports, so a reviewer changing a spec sees exactly which code leans on it — closing the loop goals ← specs ← architecture ← code, alongside specs ← executable tests.
...
- **Spec first.** For behavior or design changes, write the most-specific spec point before code; cite it from code and executable proof.
- **Refresh cited specs before editing code.** Before editing code that carries a `{MARKER}<ID>` or `{MARKER}<ID>.<section>` citation, run `grund show <ID>` for a bare ID or `grund show <ID>.<section>` for a section citation; use `grund show <ID> --toc` when you need the section map, and escalate to `grund show <ID> --full` only when the slice is not enough.
