...
- **Refresh cited specs before editing code.** Before editing code that carries a `{MARKER}<ID>` or `{MARKER}<ID>.<section>` citation, run `grund show <ID>` or `grund show <ID>.<section>` for the cited behavior and keep that output in context while making the change.
