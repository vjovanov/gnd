# grund-init-test — agent instructions
...
Declarations are heading lines `# FS-042-user-login: …` in markdown, or the same shape in a doc-comment (Javadoc, JSDoc, Rustdoc, Python docstring, Go `//`). An inline source declaration can be represented by a one-line stub in the configured kind home: `# <ID>: [<path>](<path>)`.
