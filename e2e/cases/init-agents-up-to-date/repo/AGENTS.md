...
- `grund show <ID>` — for a bare cited `§<ID>`, read the shortest useful view first.
...
Code back-references the spec it realizes. Cite the most-specific point: a whole behavior on the function/class/block doc-comment, a clause or decision (`§<ID>.2.1`) inline where it is enforced. Each citation is one more edge `grund refs <ID>` reports, so a reviewer changing a spec sees exactly which code leans on it — closing the loop goals ← specs ← architecture ← code, alongside specs ← executable tests.
