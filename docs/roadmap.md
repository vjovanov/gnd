...
An e2e fixture with a numbered-format config and a `# FS-login: …` heading gets exactly one warning naming the heading and the format, `gnd check` still exits `0`, and a sibling fixture whose heading *does* match the format gets none. `gnd list` is unchanged in both (a near-miss heading is still not a declaration).
