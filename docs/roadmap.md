...
Evaluate and, if the benchmark says it pays for itself, ship parallel per-file scanning for the hot full-tree commands. The target is not "use threads" as an implementation preference; the target is pushing large warm-cache scans toward **1M scanned lines per second** while preserving the byte-for-byte deterministic output contract ([§FS-errors.4](functional-spec/FS-errors.md#4-determinism)) and the existing scanner contract ([§AR-scanner](architecture/AR-scanner.md#ar-scanner-how-grund-discovers-declarations-and-citations)).

### 1. What

Prototype a scanner path where `walk_scannable_files` still produces a stable sorted file list, but each file is scanned independently into a per-file result. Merge those results in sorted path order, then sort any cross-file surfaces exactly as today, so `grund check`, `grund refs`, `grund list`, `grund cover`, and query commands keep byte-identical stdout/stderr on the existing e2e corpus. The first shipping target is scan/read-only commands; `fmt --write` stays sequential unless a separate measurement shows that parallel writes are worth the added ordering and failure-mode complexity.

Workspace mode gets the same treatment at two levels: member projects may be scanned independently after alias/config validation, and files inside each member may be scanned independently. The implementation must keep the one-read qualified-citation behavior described by [§AR-workspace.8](architecture/AR-workspace.md#8-downstream-commands-compose-not-duplicate) — parallelism must not regress into second disk passes.
...
The parallel scanner is accepted only if the benchmark report shows no meaningful regression on this repo and a clear large-fixture win. The aspirational headline is **1M scanned lines per second** on the 10k-file warm-cache fixture; the hard floor is still the [§GOAL-fast-feedback.1](goals.md#1-performance-targets) budget. The full e2e suite remains byte-identical, including text and JSON report ordering.
