...
- A `bench` CI job installs `valgrind` and the version-matched `iai-callgrind-runner` and runs `cargo bench --features bench --bench instructions` — [§AR-ci.5](../../architecture/AR-ci.md#5-benchmark-job). With [§RM-benchmarks](../../roadmap.md#rm-benchmarks-a-benchmark-harness-for-the-goal-fast-feedback-budgets)'s committed baseline and regression threshold, it is the authoritative [§GOAL-fast-feedback](../../goals.md#goal-fast-feedback-grund-must-be-as-fast-as-possible) meter and [§AR-ci.4](../../architecture/AR-ci.md#4-performance-smoke-guard)'s timeout stays as a backstop.
