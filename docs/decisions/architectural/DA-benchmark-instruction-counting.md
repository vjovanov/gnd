...
- A `bench` CI job installs `valgrind` and the version-matched `iai-callgrind-runner` and runs `cargo bench --features bench` — [§AS-ci.5](../../architectural-spec/AS-ci.md#5-benchmark-job). It is informational until [§RM-benchmarks](../../roadmap.md#rm-benchmarks-a-benchmark-harness-for-the-goal-fast-feedback-budgets) adds the committed baseline and the regression threshold; then it becomes the authoritative [§GOAL-fast-feedback](../../goals/goals.md#goal-fast-feedback-grund-must-be-as-fast-as-possible) meter and [§AS-ci.4](../../architectural-spec/AS-ci.md#4-performance-smoke-guard)'s timeout stays as a backstop.
...
- No `e2e/cases/*` change, no CLI surface change, no `grund_config_version` change — this is build-and-CI tooling, outside the [§GOAL-no-silent-breakage](../../goals/goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path) user-visible surface.
