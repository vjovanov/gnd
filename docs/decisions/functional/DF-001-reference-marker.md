# DF-001-reference-marker: Use § as the reference marker, with $$ as the typing trigger
...
The gnd reference scheme cites IDs as bare tokens in prose: `FS-042-user-login.3.1`. This is token-cheap but ambiguous — a stray match in unrelated text (e.g. an issue tracker code, a regex example) can be picked up by the scanner as a citation, producing false positives and false dangling-ref errors.
...
- **Live, in the gnd IDE plugins (FS-003-ide-plugins).** Type `$$FS-001-check`; `$$` becomes `§` as soon as the regex matches.
- **Bulk, via `gnd fmt` (FS-005-fmt).** Walk files and rewrite `<trigger><ID>` to `<marker><ID>`. Idempotent. Used as a pre-commit hook and a CI safety net.
...
**Default: optional.** Bare `FS-042-user-login` is still a valid citation; gnd recognizes it. `§FS-042-user-login` is preferred; tooling and editor previews use the marker form.
...
Both marker and trigger are configurable per G-006-configurable:
...
- The scanner (AS-001-scanner) recognizes both bare and marker-prefixed citations by default, and only marker-prefixed citations under `strict = true`.
- The IDE plugins (FS-003-ide-plugins) transform `$$<KIND>-<digit>` to `§<KIND>-<digit>` on the fly.
- A new functional spec, FS-005-fmt, defines `gnd fmt` for bulk transformation.
