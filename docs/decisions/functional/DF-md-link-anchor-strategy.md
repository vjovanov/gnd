...
- **[§GOAL-zero-config](../../goals.md#goal-zero-config-works-on-any-conformant-tree).** `--cross-refs` is opt-in ([§DF-md-link-emission.2.4](DF-md-link-emission.md#24-opt-in-never-default)); the `github` default fits the most common hosting case, so opting in works out-of-the-box for the majority.
