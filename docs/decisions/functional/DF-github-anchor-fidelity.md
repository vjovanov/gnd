...
All profiles slugify the heading's **rendered text**, not its source. The only case where those differ for gnd's headings is an inline link `[text](url)` in the heading — which a renderer shows as `text` — and the case bites because `gnd fmt --cross-refs` itself wraps a citation that lives in a section heading (`### 2.2 … supersede §FS-fmt.6.3` becomes `### 2.2 … supersede [§FS-fmt.6.3](…)`). So heading text is reduced — `[text](url)` → `text` — before it reaches the slugger. Two consequences fall out: a citation pointing at such a section gets the same anchor it would have had if the heading's own citation were unwrapped, and `gnd fmt --cross-refs` is idempotent over a heading-borne citation (the alternative — slugifying the raw source — made the second `fmt` pass mangle the anchor with the destination URL it had just inserted, e.g. `#22-re-derive-…-fs-fmt63functional-specfs-fmtmd63-idempotency-and-re-derive`).
...
- This repo's pre-commit hook gains `gnd fmt --cross-refs --write` and switches `lychee` to `--include-fragments`, so anchor drift in `docs/` is caught at commit time.
