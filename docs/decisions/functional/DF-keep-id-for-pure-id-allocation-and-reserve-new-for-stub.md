...
| Rename to `alloc` or `mint` | More technically precise than `new`, but less obvious than `id` and not materially better for a one-token frequent command ([§GOAL-friendliness-first.1](../../goals/goals.md#1-hard-requirements)). |
