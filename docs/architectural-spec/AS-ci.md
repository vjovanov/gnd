...
Until the criterion benchmark harness lands (§RM-benchmarks), CI carries a cheap floor on §G-fast-feedback rather than no enforcement at all: the `gnd .` self-check step runs the already-built binary under a generous wall-clock `timeout` — long enough never to flake on a loaded runner, short enough to fail the build on a catastrophic regression such as an accidental quadratic walk or a second read pass over every file. It is not the budget itself (the budget is tens of milliseconds; the ceiling is tens of seconds) — it is the difference between "we'd notice eventually" and "the build is red on the commit that did it". When §RM-benchmarks ships, the criterion job becomes the authoritative check; the timeout can stay as a backstop.
