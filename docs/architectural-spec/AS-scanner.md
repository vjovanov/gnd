...
The scanner is the single tree-walk that produces all of gnd's input data. Every check in [§FS-check](../functional-spec/FS-check.md) and every retrieval in [§FS-show](../functional-spec/FS-show.md) derives from what the scanner finds. Speed ([§G-fast-feedback](../goals/goals.md)) is set here.
...
- The skip list is configurable per [§G-configurable](../goals/goals.md) and [§FS-config.3.5](../functional-spec/FS-config.md#35-scan--what-gets-walked).
...
The marker character recognized in citations follows [§DF-reference-marker](../decisions/functional/DF-reference-marker.md); the regex shape changes when the marker is reconfigured per [§G-configurable](../goals/goals.md).
