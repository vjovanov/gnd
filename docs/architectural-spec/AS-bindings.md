...
Speaks LSP over stdio (per [§AS-lsp.4](AS-lsp.md#4-transport)). Imports `grund-core` for scan/check/show/fmt; imports `tower-lsp` (or equivalent) plus `tokio` for the protocol surface. Publishes as `grund-lsp` on every registry per [§FS-distribution.1](../functional-spec/FS-distribution.md#1-targets) and [§DA-lsp-optional](../decisions/architectural/DA-lsp-optional.md#da-lsp-optional-lsp-server-ships-as-a-separate-optional-binary). Independent of `grund-cli` — neither pulls the other in. The full architecture lives in [§AS-lsp](AS-lsp.md#as-lsp-how-the-lsp-server-is-built).
