...
Implements the planned distribution shape in [§FS-distribution](../functional-spec/FS-distribution.md#fs-distribution-gnd-distribution-targets). Target state: the repo is a Cargo workspace with one core library and four frontends — three for batch use (CLI, Node, Python) and one for editor use (LSP). The current implementation is still a single Rust crate; this architectural spec describes the split that must happen before the LSP and language bindings ship.
...
- `gnd::refs(findings: &Findings, id: &str, section: Option<&str>) -> Vec<Citation>` ([§FS-refs](../functional-spec/FS-refs.md#fs-refs-gnd-lists-every-citation-of-an-id))
