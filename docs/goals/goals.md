...
On this repo, the compact first read for a large spec should stay under 1 KB while still listing enough section structure to choose a section, against ~15 KB for the full body; `grund refs <ID> --summary` and `grund list --kind <K1>,<K2>` are a fraction of their un-narrowed forms. E2E fixtures should cover the `show --toc` capability, the bare `show` default, the retained explicit `show` slices, `refs --summary`, and `list --kind`/`--summary` in text and JSON, and an `init-agents-*` fixture should prove the generated managed block teaches the default/section/full ladder. Roadmap: [§RM-token-cheap-grounding](../roadmap.md#rm-token-cheap-grounding-token-cheap-read-surfaces-for-agents).
