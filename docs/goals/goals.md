...
An agent grounds itself by pulling cited facts into a finite context window, and every token spent re-reading something it didn't need is a token it can't spend on the work. So the rule, across every read and query surface: return the *smallest* output that actually answers the question, and make the cheaper move the obvious one. The goal is the best observed behavior for humans and agents, not loyalty to the first flag shape: keep the capabilities explicit, change defaults when evidence shows a better first move, and record what we learn as research notes (§4). An agent landing on a bare `§<ID>` should not have to load a 15 KB spec body to discover whether it is looking at the right declaration — the default should be the shortest useful read. When the reader needs more orientation, `grund show <ID> --toc` should add top context plus section coordinates before the agent fetches one section or reaches for the full body. This is the agent-facing half of [§GOAL-friendliness-first](goals.md#goal-friendliness-first-as-user--and-agent-friendly-as-possible) and what makes [§GOAL-agent-grounding](goals.md#goal-agent-grounding-agents-stay-cited-as-they-work) affordable in practice: grounding that costs less gets done more.

### 1. What this requires

- **A cheap default, with an escalation path.** For a declaration, the best default is still being explored through actual use and measured output. Keep separate flags for distinct capabilities while changing the default toward the cheapest useful read. The current hypothesis is that bare `grund show <ID>` should be the shortest useful declaration view, while the table-of-contents capability is named `--toc`: more detail through top context plus the section coordinates an agent needs before choosing `grund show <ID>.<section>` (one section — [§FS-show.2.2](../functional-spec/FS-show.md#22-section)), with `grund show <ID> --full` held for when the slice falls short. `--head`, `--outline`, and `--brief` remain available as narrower capability selectors, not as the only way to avoid the expensive path. For back-references: `grund refs <ID> --summary` (one line per file — [§FS-refs.3.3](../functional-spec/FS-refs.md#33---summary)) before the full per-citation list. For discovery: `grund list --kind FS,AR` / `grund list --summary` ([§FS-list.3.3](../functional-spec/FS-list.md#33---summary)) before the full catalog.
- **The generated entrypoint teaches the default.** The `AGENTS.md` managed block should teach bare `grund show <ID>` as the shortest first read once that default is specified, `grund show <ID> --toc` when the reader needs section navigation, `grund show <ID>.<section>` for a section citation, and `grund show <ID> --full` for the whole body when the slice is not enough ([§FS-init.2.3](../functional-spec/FS-init.md#23-generated-agent-entrypoints)).
- **The generated entrypoint stays concise.** `AGENTS.md` is read at session start, so its managed block should teach only the rules an agent needs before work begins, in compact wording; detail belongs in cited specs and `grund show` output.
...
On this repo, the shortest first read for a large spec should stay well under 1 KB, `grund show <ID> --toc` should stay compact while listing enough section structure to choose a section, and both should remain far below the ~15 KB full body. `grund refs <ID> --summary` and `grund list --kind <K1>,<K2>` are a fraction of their un-narrowed forms. E2E fixtures should cover the bare `show` default, the `show --toc` capability, the retained explicit `show` slices, `refs --summary`, and `list --kind`/`--summary` in text and JSON, and an `init-agents-*` fixture should prove the generated managed block teaches the default/toc/section/full ladder. Roadmap: [§RM-token-cheap-grounding](../roadmap.md#rm-token-cheap-grounding-token-cheap-read-surfaces-for-agents).

### 4. Research notes

- **Full-body default burns tokens.** Treating bare `grund show <ID>` as `--full` made the common bare-citation workflow expensive: an agent often needed only the top context and section coordinates, but paid for the whole declaration body first. On larger specs that turned a navigation step into ~15 KB of prompt load before the agent could choose a section. This is the evidence behind exploring a compact default plus explicit `--full`.
- **Shortest default, TOC on demand.** The current research direction is that bare `grund show <ID>` should answer the first and cheapest question — "what is this declaration?" — with the shortest useful output. `grund show <ID> --toc` is the follow-up when that answer is not enough and the reader needs section coordinates before fetching one section.
