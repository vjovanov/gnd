...
- **Show is grounded.** `grund show <ID>` returns just the declaration body — no surrounding context, no scrolling, no token waste — under 200 lines for the common case, with cheaper slices below it ([§GOAL-token-economy](goals.md#goal-token-economy-give-an-agent-the-right-amount-of-spec-not-the-whole-file)).
...
An agent grounds itself by pulling cited facts into a finite context window, and every token spent re-reading something it didn't need is a token it can't spend on the work. So the rule, across every read and query surface: return the *smallest* output that actually answers the question, and make the cheaper move the obvious one. An agent landing on a bare `§<ID>` should not have to load a 15 KB spec body to discover which section it wants — it should be able to ask for the lead paragraph and the section map, then fetch the one section, and reach for the full body only when the slice falls short. This is the agent-facing half of [§GOAL-friendliness-first](goals.md#goal-friendliness-first-as-user--and-agent-friendly-as-possible) and what makes [§GOAL-agent-grounding](goals.md#goal-agent-grounding-agents-stay-cited-as-they-work) affordable in practice: grounding that costs less gets done more.

### 1. What this requires

- **A cheap first move, with an escalation path.** For a declaration: `grund show <ID> --brief` (lead prose + section map — [§FS-show.2.1.3](../functional-spec/FS-show.md#213-brief---brief)) for a bare cited ID — or `grund show <ID> --outline` (the map alone — [§FS-show.2.1.2](../functional-spec/FS-show.md#212-outline-only---outline)) when the declaration is already familiar and only the section is in question — then `grund show <ID>.<section>` (one section — [§FS-show.2.2](../functional-spec/FS-show.md#22-section)), with `grund show <ID>` (the whole body) held for when the slice falls short. For back-references: `grund refs <ID> --summary` (one line per file — [§FS-refs.3.3](../functional-spec/FS-refs.md#33---summary)) before the full per-citation list. For discovery: `grund list --kind FS,AR` / `grund list --summary` ([§FS-list.3.3](../functional-spec/FS-list.md#33---summary)) before the full catalog.
- **The generated entrypoint teaches the ladder.** The `AGENTS.md` managed block names `--brief` as the first read for a bare cited ID and `<ID>.<section>` for a section citation, with the full body as the escalation — not `grund show <ID>` as the reflexive first move ([§FS-init.2.3](../functional-spec/FS-init.md#23-generated-agent-entrypoints)).
...
On this repo, the recommended first read for a large spec (`grund show FS-check --brief`) is under 1 KB while still listing enough section structure to choose a section, against ~15 KB for the full body; `grund refs <ID> --summary` and `grund list --kind <K1>,<K2>` are a fraction of their un-narrowed forms. E2E fixtures cover `show --outline` / `show --brief`, `refs --summary`, and `list --kind`/`--summary` in text and JSON, and an `init-agents-*` fixture proves the generated managed block teaches the brief/section-first ladder. Roadmap: [§RM-token-cheap-grounding](../roadmap.md#rm-token-cheap-grounding-token-cheap-read-surfaces-for-agents).
