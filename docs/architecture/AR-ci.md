...
On pull requests, the job checks out the PR base branch and runs `cargo bench --features bench --locked --bench instructions -- --save-baseline=main --save-summary=json`; then it checks out the PR commit and runs `cargo bench --features bench --locked --bench instructions -- --baseline=main --callgrind-limits=ir=5.0% --save-summary=json`. The named baseline comes from the current base branch rather than a stale generated artifact, while the committed baseline figures in `docs/benchmarks.md` remain the human-readable reference point. Any instruction-count growth beyond 5% is a build failure for [§GOAL-fast-feedback.3](../goals.md#3-measurable). On pushes to `main`, the job records the current counts and saves JSON summaries without comparing against itself.
