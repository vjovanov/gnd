...
- **A cheap default, with an escalation path.** Bare `grund show <ID>` is the cheapest useful declaration read: lead prose, cut at the first child section heading. `grund show <ID> --brief` is the smaller hover-sized slice; `grund show <ID> --toc` adds the section coordinates an agent needs before choosing `grund show <ID>.<section>` (one section — [§FS-show.2.2](functional-spec/FS-show.md#22-section)); `grund show <ID> --full` is held for when the narrower slices fall short. For back-references: `grund refs <ID> --summary` (one line per file — [§FS-refs.3.3](functional-spec/FS-refs.md#33---summary)) before the full per-citation list. For discovery: `grund list --kind FS,AR` / `grund list --summary` ([§FS-list.3.3](functional-spec/FS-list.md#33---summary)) before the full catalog.
...
- **Shortest default, TOC on demand.** Bare `grund show <ID>` answers the first and cheapest question — "what is this declaration?" — with the shortest useful output. `grund show <ID> --toc` is the follow-up when that answer is not enough and the reader needs section coordinates before fetching one section.
