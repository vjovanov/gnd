...
Manual timing on this repo and on a synthetic 10k-file fixture should stay within the targets above. The `cargo bench` instruction-counting harness over the commands agents and CI run most ([§AR-benchmarks](architecture/AR-benchmarks.md#ar-benchmarks-instruction-counting-benchmarks-for-the-hot-cli-commands), run in CI by the [§AR-ci.5](architecture/AR-ci.md#5-benchmark-job) job) records the per-commit number and fails pull requests that regress beyond the configured threshold from the base-branch baseline ([§RM-benchmarks](roadmap.md#rm-benchmarks-a-benchmark-harness-for-the-goal-fast-feedback-budgets)). Alongside it CI runs the built `grund .` under a generous timeout ([§AR-ci.4](architecture/AR-ci.md#4-performance-smoke-guard)) so a catastrophic regression — an accidental quadratic walk, a re-read pass — still fails the build outright.
...
- **Show is grounded.** `grund <ID>` returns one declaration or a structural slice of it — no surrounding file context, no scrolling, no token waste — under 200 lines for the common case, with the default biased toward the cheapest useful read ([§GOAL-token-economy](goals.md#goal-token-economy-give-an-agent-the-right-amount-of-spec-not-the-whole-file)).
...
Typical `grund <ID>` output under 200 lines; `grund <ID> --format=json` validates against a stable schema in `e2e/`; `grund --help` fits in 24 lines and lists each frequent command as one token; round-trip determinism is enforced by an e2e test that runs `grund` twice and diffs the output.

## GOAL-token-economy: give an agent the right amount of spec, not the whole file

An agent grounds itself by pulling cited facts into a finite context window, and every token spent re-reading something it didn't need is a token it can't spend on the work. So the rule, across every read and query surface: return the *smallest* output that actually answers the question, and make the cheaper move the obvious one. The goal is the best observed behavior for humans and agents, not loyalty to the first flag shape: keep the capabilities explicit, change defaults when evidence shows a better first move, and record what we learn as research notes (§4). An agent landing on a bare `§<ID>` should not have to load a 15 KB spec body to discover whether it is looking at the right declaration — the default should be the shortest useful read. When the reader needs more orientation, `grund <ID> --toc` should add top context plus section coordinates before the agent fetches one section or reaches for the full body. This is the agent-facing half of [§GOAL-friendliness-first](goals.md#goal-friendliness-first-as-user--and-agent-friendly-as-possible) and what makes [§GOAL-agent-grounding](goals.md#goal-agent-grounding-agents-stay-cited-as-they-work) affordable in practice: grounding that costs less gets done more.

### 1. What this requires

- **A cheap default, with an escalation path.** Bare `grund <ID>` is the cheapest useful declaration read: lead prose, cut at the first child section heading. `grund <ID> --brief` is the smaller hover-sized slice; `grund <ID> --toc` adds the section coordinates an agent needs before choosing `grund <ID>.<section>` (one section — [§FS-show.2.2](functional-spec/FS-show.md#22-section)); `grund <ID> --full` is held for when the narrower slices fall short. For back-references: `grund refs <ID> --summary` (one line per file — [§FS-refs.3.3](functional-spec/FS-refs.md#33---summary)) before the full per-citation list. For discovery: `grund list --kind FS,AR` / `grund list --summary` ([§FS-list.3.3](functional-spec/FS-list.md#33---summary)) before the full catalog.
- **The generated entrypoint teaches the default.** The `AGENTS.md` managed block should teach bare `grund <ID>` as the shortest first read once that default is specified, `grund <ID> --toc` when the reader needs section navigation, `grund <ID>.<section>` for a section citation, and `grund <ID> --full` for the whole body when the slice is not enough ([§FS-init.2.3](functional-spec/FS-init.md#23-generated-agent-entrypoints)).
- **The generated entrypoint stays concise.** `AGENTS.md` is read at session start, so its managed block should teach only the rules an agent needs before work begins, in compact wording; detail belongs in cited specs and `grund <ID>` output.
- **One-token commands.** The frequent read/query verbs stay invokable as a single shell token after `grund` ([§GOAL-friendliness-first.1](goals.md#1-hard-requirements)) and the flag names that turn up in those loops cost no more tokens than the verbs they modify.
- **Slices are structural, never generated.** Every cheap output is a verbatim subset of bytes `grund` already parsed — heading lines, lead prose, per-file counts — so it stays byte-deterministic on `(tree, config)`. No model-written summaries, and `grund check` is never abridged to save tokens: diagnostics stay complete ([§FS-non-goals.14](functional-spec/FS-non-goals.md#14-generated-summaries-token-saving-inside-check)).

### 2. What this rules out

A read surface that forces the agent to take the expensive path — a `show` with no sub-body slice, a `refs` that can only print every column, a `list` that can only dump everything — and any "token-saving" that trades completeness or determinism for brevity: an LLM-generated précis, a `check` that hides findings. The token economy is won by exposing the structure `grund` already knows, not by guessing what the reader can do without.

### 3. Measurable

On this repo, the shortest first read for a large spec should stay well under 1 KB, `grund <ID> --toc` should stay compact while listing enough section structure to choose a section, and both should remain far below the ~15 KB full body. `grund refs <ID> --summary` and `grund list --kind <K1>,<K2>` are a fraction of their un-narrowed forms. E2E fixtures should cover the bare-ID default, the `--toc` capability, the retained explicit `show` slices, `refs --summary`, and `list --kind`/`--summary` in text and JSON, and an `init-agents-*` fixture should prove the generated managed block teaches the default/toc/section/full ladder. Roadmap: [§RM-token-cheap-grounding](roadmap.md#rm-token-cheap-grounding-token-cheap-read-surfaces-for-agents).

### 4. Research notes

- **Full-body default burns tokens.** Treating the default ID read as `--full` made the common bare-citation workflow expensive: an agent often needed only the top context and section coordinates, but paid for the whole declaration body first. On larger specs that turned a navigation step into ~15 KB of prompt load before the agent could choose a section. This is the evidence behind the compact default plus explicit `--full`.
- **Shortest default, TOC on demand.** Bare `grund <ID>` answers the first and cheapest question — "what is this declaration?" — with the shortest useful output. `grund <ID> --toc` is the follow-up when that answer is not enough and the reader needs section coordinates before fetching one section.
