# DISC-skill-init-existing-specs: `grund-init` skill behavior for repos that already have specs

## Status

Discussion. Candidate follow-up for [§FS-init.5](../../functional-spec/FS-init.md#5-agent-setup-instructions) and `skills/grund-init/SKILL.md`.
...
1. **Inventory first.** Detect existing spec homes, naming patterns, ID-like headings, citation markers, decisions, roadmaps, changelogs, e2e fixtures, and agent instruction files before recommending any command. This is already the first step in [§FS-init.5](../../functional-spec/FS-init.md#5-agent-setup-instructions); the discussion here is about making its consequence explicit.
...
- Should `skills/grund-init/SKILL.md` have an explicit "existing specs" subsection with the decision table above, or is [§FS-init.5](../../functional-spec/FS-init.md#5-agent-setup-instructions) enough once tightened?
- Should the skill summarize "unmapped spec-like files" after config generation, so the user can decide whether to add more `[[kinds]]` entries or exclude noisy folders?
- Should the default be `strict = true` when all detected citations already use the marker form, or should adoption still bias toward `strict = false` until the first clean `grund check`?
- Should a repo with ADRs/RFCs but no functional specs get custom `ADR`/`RFC` kinds immediately, or should the skill keep canonical kinds and map those documents under `DF`/`DA` only when their semantics match?
- Should the skill offer a second-phase migration plan template after successful init, or leave that to the user request that follows?

## Working opinion

The first setup pass should be conservative: preserve the user's existing spec system, map it into `grund` config, and make the agent entrypoint teach the resulting layout. `--docs` is correct for an empty repo and usually wrong for a repo that already has specs. The value of the skill is not that it can scaffold files; it is that it can inspect a real project and choose the smallest non-surprising adoption step.
