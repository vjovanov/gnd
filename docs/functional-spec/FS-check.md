...
- **Scope.** `--watch` will be a `check` flag (and `grund --watch [<path>]` will be shorthand for `grund check --watch [<path>]`, [§FS-cli](FS-cli.md#fs-cli-grunds-command-line-surface-conventions)). Other subcommands will not take it; a one-shot `grund fmt` or `grund show` has nothing to keep watching.
