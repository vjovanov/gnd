...
- `gnd <subcommand> …` dispatches to that subcommand: `check`, `show`, `list`, `refs`, `cover`, `fmt`, `init`, `name`, `config`, `completions`. The hidden `complete` subcommand is reserved for generated shell scripts (§FS-completions.2).
