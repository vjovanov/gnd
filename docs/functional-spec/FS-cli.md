...
- `--format text|json` — accepted by the subcommands with a machine-readable result or diagnostic surface: `check`, `show`, `list`, `refs`, `cover`, and `name` (§FS-errors.5). `text` is the default; `json` opts into the stable machine shapes (diagnostic NDJSON on stderr, result object or records on stdout — §FS-errors.5, §FS-distribution.3.0). It is not a global flag: operational commands whose output is human text or generated files (`fmt`, `init`, `config`, `agent-setup-instructions`, `completions`) reject `--format` unless their own help page says otherwise.
