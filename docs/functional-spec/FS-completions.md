...
```
gnd complete ids [<path>] [--prefix <prefix>] [--sections] [--path <path>]
```
...
The helper uses the same config discovery and scanner as `gnd list`, `gnd show`, and `gnd refs`; a clean repo therefore has one shared declaration catalog across listing, retrieval, reverse lookup, and completion. Candidate ordering is deterministic for a given tree and config.
...
```bash
source <(gnd completions bash)
gnd completions zsh > ~/.zfunc/_gnd
gnd completions fish > ~/.config/fish/completions/gnd.fish
```

Packaging may install these scripts automatically later, but the CLI contract is the generated bytes from `gnd completions <shell>`.
