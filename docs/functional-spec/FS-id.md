...
- `1` — slug empty (§3) or collision detected (§5). These are query failures — `id` had a well-formed request but cannot return an ID — so they print a bare line on stderr, no `error:` prefix, the same convention `gnd show` uses for `ID not found` ([§FS-errors.2.3](FS-errors.md#23-bare-query-result)).
