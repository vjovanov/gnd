...
The `show` subcommand prints just the body of a declaration, given an ID. It exists so an agent — human or AI — can pull a single grounded fact into context without loading the whole file. Serves G-005-friendliness-first.
...
- `<ID>` — the full ID (e.g. `FS-001-check`). May include an inline section (`FS-001-check.3.1`). The dotted form uses the configured `[id] section_separator` (FS-006-config.3.2). When the separator is non-default (e.g., `:` or `#`) the inline form may collide with the slug grammar; use `--section` instead.
...
`gnd show FS-001-check.3.1` prints just the contents under section heading `### 3.1 ...` within the declaration body, stopping at the next sibling-or-shallower heading. Nested deeper headings (e.g., `#### 3.1.2`) are included in the output — they end at the next `### 3.x` (sibling) or `## N.` (shallower) heading. Arbitrary nesting depth is supported per FS-006-config.3.3.
...
A declaration is found on a "declaration line" — a line that matches the declaration regex from AS-001-scanner.2.1 *and* sits inside a comment or docstring. The block surrounding it is computed as follows:
...
Without `show`, an agent retrieving a spec section either loads the whole file (token-expensive) or reimplements the parser. With `show`, the canonical way to pull `FS-001-check.3.1` into a prompt is exactly:

```
gnd show FS-001-check.3.1
```
