...
Without `init`, the on-ramp to `gnd` is a copy-paste of someone else's `AGENTS.md`, with the resulting drift between projects. `init` collapses the on-ramp to one command and freezes the canonical entry point at the `gnd` version that wrote it — when the canonical form evolves, `gnd init --force` re-emits it ([§G-configurable](../goals/goals.md) still applies for downstream edits).
