...
The integer after `v` is the agent guidance block schema version. A fresh `AGENTS.md` consists of this block. If `AGENTS.md` already exists and contains no `gnd:init:agents` block, `init` appends the current block after the existing content. If a known companion entrypoint such as `AGENTS.override.md`, `CLAUDE.md`, `.claude/CLAUDE.md`, `GEMINI.md`, or `.github/copilot-instructions.md` exists and is not a symlink to `AGENTS.md`, the same append/update rules apply to that companion. If the file already contains any supported block version, including the current one, `init` replaces only the bytes between the begin and end markers with the current block rendered from the current effective `.agents/gnd.toml` and leaves all content before and after the block untouched. This means same-version template/config changes propagate on the next `gnd init` without requiring `--force`. If the file contains a newer block version than the running binary supports, `init` exits 2 and leaves the file unchanged.
