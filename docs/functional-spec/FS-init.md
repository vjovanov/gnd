...
The block describes the configured declaration homes from the effective `.agents/grund.toml`, so the generated instructions name the host repo's actual artifact layout instead of hard-coding the default layout. The scan scope (`[scan].include` / `[scan].exclude`) is *not* surfaced here — it is configuration an agent never needs to read inline, since `grund show`, `grund list`, and `grund refs` apply it transparently.
