...
- §FS-errors.3: the worked example for the "lowercase first letter" rule used `dangling citation: FS-foo`, which is not the text `gnd` emits (`unknown reference FS-foo`); the example now matches the shipped message.
