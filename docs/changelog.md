...
- [§FS-init.5](functional-spec/FS-init.md#5-agent-setup-instructions) / [§DF-skill-init-existing-specs](decisions/functional/DF-skill-init-existing-specs.md#df-skill-init-existing-specs-grund-init-adopts-existing-specs-before-scaffolding): clarify that the `grund-init` skill asks users to choose an artifact model before adopting repos that already have specs, covering canonical `grund` with the `GRUND` grounding tag, canonical core plus project-specific extras, and existing structure with citations. PR #17.
