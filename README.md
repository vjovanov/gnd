...
5. If `agents.md` is present, it carries an up-to-date `gnd init` block; any known standalone companion entrypoints such as `AGENTS.md`, `AGENTS.override.md`, `CLAUDE.md`, `.claude/CLAUDE.md`, `GEMINI.md`, or `.github/copilot-instructions.md` are checked only in that scaffolded-entrypoint mode. *(uninitialized / stale agent entry point — run `gnd init`)*
...
| `gnd agent-setup-instructions` | Print the AI-agent setup guide embedded from `skills/gnd-init/SKILL.md`, so agents with only the installed binary can still perform guided `gnd` adoption. |
